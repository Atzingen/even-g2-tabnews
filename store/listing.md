# TabNews para Even G2 — textos da listagem

## Nome (app.json, máx. 20 caracteres)
...
Swipe: move o cursor · Toque: abre · Duplo toque: volta ou sai
Swipe: move cursor · Tap: open · Double tap: back or exit

## Permissões

- `network` — Lê as notícias públicas do TabNews (tabnews.com.br). / Reads public TabNews posts (tabnews.com.br).

## Política de privacidade (texto para publicar numa URL)
...
**PT:** Primeira versão: posts relevantes e recentes dos últimos 5 dias, leitor paginado, troca de modo com um toque.

**EN:** First release: relevant and newest posts from the last 5 days, paged reader, one-tap mode switch.

## Categoria sugerida

News / Reading

## Screenshots (576x288, PNG RGBA transparente)

O review de 10/09/2026 rejeitou o fundo preto. Usar as capturas nativas abaixo,
feitas em 12/09/2026, preservando o canal alpha. O portal aplica a foto de ambiente
atrás do conteúdo. Não converter para RGB nem adicionar fundo preto.

1. `screenshots-rgba/01-relevantes.png` — lista no modo Relevantes.
2. `screenshots-rgba/02-selecao.png` — outro post selecionado.
3. `screenshots-rgba/03-leitor.png` — conteúdo do post no leitor.
4. `screenshots-rgba/04-leitor-seguinte.png` — página seguinte.
5. `screenshots-rgba/05-recentes.png` — lista no modo Recentes.

Os PNGs antigos `screenshot-*.png` têm fundo preto e foram preservados apenas
como histórico; não devem ser reenviados.

Fonte: https://hub.evenrealities.com/docs/test/simulator#screenshot

### Reenvio em 12/09/2026

As três imagens com fundo preto foram substituídas no portal pelas cinco
capturas RGBA acima. A prévia foi conferida com o texto sobre a foto de
